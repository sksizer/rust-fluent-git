...
impl<'a> BranchBuilder<'a> {
    ...
    pub fn create(self, name: impl Into<String>) -> BranchCreateBuilder<'a> {
        BranchCreateBuilder { repo_path: self.repo_path, name: name.into() }
    }

    /// Delete a branch.
    pub fn delete(self, name: impl Into<String>) -> BranchDeleteBuilder<'a> {
        BranchDeleteBuilder { repo_path: self.repo_path, name: name.into() }
    }

    /// List branches.
    pub fn list(self) -> BranchListBuilder<'a> {
        BranchListBuilder { repo_path: self.repo_path }
    }

    /// Rename a branch.
    pub fn rename(self, old: impl Into<String>, new: impl Into<String>) -> BranchRenameBuilder<'a> {
        BranchRenameBuilder { repo_path: self.repo_path, old_name: old.into(), new_name: new.into() }
    }
...
impl<'a> BranchCreateBuilder<'a> {
    pub(crate) fn build_command(&self) -> ShellCommand {
        ShellCommand::new("git").arg("-C").arg(self.repo_path.to_string_lossy().as_ref()).arg("branch").arg(&self.name)
    }
...
pub(crate) fn parse_create_output(output: &Output, name: &str) -> Result<(), BranchError> {
    ...
    if lower.contains("already exists") {
        return Err(BranchError::AlreadyExists { name: name.to_string() });
    }

    if lower.contains("not a valid branch name") || lower.contains("invalid branch name") {
        return Err(BranchError::InvalidName { name: name.to_string(), reason: stderr.clone() });
    }
...
pub(crate) fn parse_delete_output(output: &Output, name: &str) -> Result<(), BranchError> {
    ...
    if lower.contains("checked out")
        ...
        || lower.contains("cannot delete branch") && lower.contains("checked out")
    {
        return Err(BranchError::DeleteCurrent { name: name.to_string() });
    }

    if lower.contains("not fully merged") {
        return Err(BranchError::NotFullyMerged { name: name.to_string() });
    }

    if lower.contains("not found") {
        return Err(BranchError::NotFound { name: name.to_string() });
    }
...
impl<'a> BranchListBuilder<'a> {
    pub(crate) fn build_command(&self) -> ShellCommand {
        ShellCommand::new("git").arg("-C").arg(self.repo_path.to_string_lossy().as_ref()).arg("branch").arg("-vv")
    }
}
...
pub(crate) fn parse_list_output(output: &Output) -> Result<Vec<BranchInfo>, BranchError> {
    ...
    for line in stdout.lines() {
        ...
        branches.push(BranchInfo { name, is_current, upstream, sha, last_commit_message: commit_msg });
    }
...
pub(crate) fn parse_rename_output(output: &Output, old_name: &str) -> Result<(), BranchError> {
    ...
    if lower.contains("not found") || lower.contains("did not match") {
        return Err(BranchError::NotFound { name: old_name.to_string() });
    }
