//! Async `open` — verify a path is a git repository and return a `Repo`.
...
/// Open an existing git repository at `path` (async).
///
...
/// absolute path.
pub async fn open(path: impl AsRef<Path>) -> Result<Repo, OpenError> {
