...
//!   - async: fluent_git::git::*        / .run().await
//!
...
mod tests {
    ...
    async fn async_add_single_file() {
        ...
        fs::write(dir.join("hello.txt"), "hello").unwrap();
        let result: Result<AddResult, AddError> = repo.add().path("hello.txt").run().await;
        assert_eq!(result.unwrap().files.len(), 1);
    }
    ...
    async fn async_add_multiple_files() {
        ...
        fs::write(dir.join("a.txt"), "a").unwrap();
        fs::write(dir.join("b.txt"), "b").unwrap();
        let result = repo.add().path("a.txt").path("b.txt").run().await.unwrap();
        assert_eq!(result.files.len(), 2);
    }
    ...
    async fn async_commit_amend() {
        ...
        let result = repo.commit().amend().message("amended").run().await.unwrap();
        assert_eq!(result.message, "amended");
    }
    ...
    async fn async_branch_create_list_delete_rename() {
        ...
        repo.branch().create("feature").run().await.unwrap();
        repo.branch().create("to-rename").run().await.unwrap();

        let branches: Result<Vec<BranchInfo>, BranchError> = repo.branch().list().run().await;
        let branches = branches.unwrap();
        assert!(branches.len() >= 3);
        assert_eq!(branches.iter().find(|b| b.is_current).unwrap().name, "main");

        repo.branch().rename("to-rename", "renamed").run().await.unwrap();
        repo.branch().delete("feature").run().await.unwrap();

        let current: Result<String, BranchError> = repo.branch().current().await;
        assert_eq!(current.unwrap(), "main");
    }
    ...
    async fn async_branch_duplicate_fails() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        repo.branch().create("x").run().await.unwrap();
    ...
    async fn async_delete_current_branch_fails() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        assert!(matches!(repo.branch().delete("main").run().await.unwrap_err(), BranchError::DeleteCurrent { .. }));
    }
    ...
    async fn async_checkout_branch_and_new_branch() {
        ...
        repo.commit().message("init").run().await.unwrap();
        repo.branch().create("feature").run().await.unwrap();
        let result: Result<(), CheckoutError> = repo.checkout().branch("feature").run().await;
        result.unwrap();
        assert_eq!(repo.branch().current().await.unwrap(), "feature");
        repo.checkout().new_branch("feature2").run().await.unwrap();
    ...
    async fn async_checkout_nonexistent_fails() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        assert!(matches!(repo.checkout().branch("nope").run().await.unwrap_err(), CheckoutError::RefNotFound { .. }));
    }
    ...
    async fn async_status() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        let status: Result<StatusResult, StatusError> = repo.status().run().await;
        assert!(status.unwrap().is_clean());

        fs::write(dir.join("h.txt"), "modified").unwrap();
        assert!(!repo.status().run().await.unwrap().is_clean());
    ...
    async fn async_status_short() {
        ...
        let repo = fluent_git::git::init(&dir).run().await.unwrap().into_repo();
        fs::write(dir.join("h.txt"), "h").unwrap();
        assert!(!repo.status().short().run().await.unwrap().is_clean());
    }
    ...
    async fn async_log() {
        ...
        fs::write(dir.join("b.txt"), "b").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("second").run().await.unwrap();
        let commits: Result<Vec<LogEntry>, LogError> = repo.log().run().await;
        let commits = commits.unwrap();
        assert_eq!(commits.len(), 2);
        assert_eq!(commits[0].message, "second");
    }
    ...
    async fn async_log_with_limit() {
        ...
        for i in 0..5 {
            fs::write(dir.join(format!("{i}.txt")), format!("{i}")).unwrap();
            repo.add().all().run().await.unwrap();
            repo.commit().message(&format!("commit {i}")).run().await.unwrap();
        }
    ...
    async fn async_log_on_empty_repo_fails() {
        ...
        let repo = fluent_git::git::init(&dir).run().await.unwrap().into_repo();
        assert!(matches!(repo.log().run().await.unwrap_err(), LogError::NoCommits));
    }
    ...
    async fn async_diff() {
        ...
        repo.commit().message("init").run().await.unwrap();
        fs::write(dir.join("h.txt"), "mod").unwrap();
        let diff: Result<DiffResult, DiffError> = repo.diff().run().await;
        assert!(!diff.unwrap().is_empty());
    }
    ...
    async fn async_diff_between_refs() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("first").run().await.unwrap();
        repo.checkout().new_branch("feature").run().await.unwrap();
        fs::write(dir.join("h.txt"), "changed").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("second").run().await.unwrap();
    ...
    async fn async_diff_bad_ref_fails() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        assert!(matches!(repo.diff().between("nope", "HEAD").run().await.unwrap_err(), DiffError::RefNotFound { .. }));
    }
    ...
    async fn async_stash_push_and_pop() {
        ...
        repo.commit().message("init").run().await.unwrap();
        fs::write(dir.join("h.txt"), "mod").unwrap();
        repo.stash().push().run().await.unwrap();
        assert_eq!(fs::read_to_string(dir.join("h.txt")).unwrap(), "h");
        repo.stash().pop().run().await.unwrap();
        assert_eq!(fs::read_to_string(dir.join("h.txt")).unwrap(), "mod");
    }
    ...
    async fn async_stash_list() {
        ...
        repo.commit().message("init").run().await.unwrap();
        fs::write(dir.join("h.txt"), "c1").unwrap();
        repo.stash().push().message("first").run().await.unwrap();
        fs::write(dir.join("h.txt"), "c2").unwrap();
        repo.stash().push().message("second").run().await.unwrap();
        let stashes: Result<Vec<StashEntry>, StashError> = repo.stash().list().run().await;
        assert_eq!(stashes.unwrap().len(), 2);
    }
    ...
    async fn async_stash_on_clean_fails() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        assert!(matches!(repo.stash().push().run().await.unwrap_err(), StashError::NothingToStash));
    }
    ...
    async fn async_tag_lifecycle() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        repo.tag().create("v0.1.0").run().await.unwrap();
        repo.tag().create("v0.2.0").message("annotated").run().await.unwrap();
        let tags = repo.tag().list().run().await.unwrap();
        assert!(tags.iter().any(|t| t.name == "v0.1.0" && !t.annotated));
        assert!(tags.iter().any(|t| t.name == "v0.2.0" && t.annotated));
        assert!(matches!(repo.tag().create("v0.1.0").run().await.unwrap_err(), TagError::AlreadyExists { .. }));
        repo.tag().delete("v0.1.0").run().await.unwrap();
    ...
    async fn async_reset() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("second").run().await.unwrap();
        let r = repo.reset().soft().to("HEAD~1").run().await.unwrap();
        assert_eq!(r.mode, ResetMode::Soft);
    }
    ...
    async fn async_merge_no_ff() {
        ...
        fs::write(dir.join("f.txt"), "f").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("feature").run().await.unwrap();
        repo.checkout().branch("main").run().await.unwrap();
        let result = repo.merge().branch("feature").no_ff().run().await.unwrap();
        assert!(!result.fast_forward);
    }
    ...
    async fn async_merge_conflict() {
        ...
        let repo = fluent_git::git::init(&dir).initial_branch("main").run().await.unwrap().into_repo();
        repo.config().set("user.name", "T").run().await.unwrap();
        repo.config().set("user.email", "t@t").run().await.unwrap();
        fs::write(dir.join("f.txt"), "original").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        repo.checkout().new_branch("a").run().await.unwrap();
        fs::write(dir.join("f.txt"), "from a").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("a").run().await.unwrap();
        repo.checkout().branch("main").run().await.unwrap();
        fs::write(dir.join("f.txt"), "from main").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("main").run().await.unwrap();
        let err: MergeError = repo.merge().branch("a").run().await.unwrap_err();
        assert!(matches!(err, MergeError::Conflict { .. }));
    }
    ...
    async fn async_cherry_pick() {
        ...
        fs::write(dir.join("f.txt"), "cherry").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("cherry this").run().await.unwrap();
        let sha = &repo.log().max_count(1).run().await.unwrap()[0].sha;
        repo.checkout().branch("main").run().await.unwrap();
        repo.cherry_pick().commit(sha).run().await.unwrap();
        assert!(dir.join("f.txt").exists());
    }
    ...
    async fn async_config_set_get_unset() {
        ...
        assert_eq!(repo.config().get("user.name").run().await.unwrap(), "Test");
        repo.config().unset("user.name").run().await.unwrap();
    ...
    async fn async_rev_parse() {
        ...
        repo.add().all().run().await.unwrap();
        repo.commit().message("init").run().await.unwrap();
        let sha: Result<String, RevParseError> = repo.rev_parse("HEAD").run().await;
        assert_eq!(sha.unwrap().len(), 40);
        assert!(matches!(repo.rev_parse("nonexistent").run().await.unwrap_err(), RevParseError::RefNotFound { .. }));
    }
    ...
    async fn async_worktree_lifecycle() {
        ...
        // Lock & unlock
        let lock = repo.worktree().lock(&wt_b).reason("external drive").run().await.unwrap();
        assert!(!lock.was_already_locked);
    ...
    async fn async_question_mark_propagation() {
        async fn workflow(repo: &Repo) -> Result<CommitResult, GitError> {
            ...
            repo.commit().message("add").run().await?;
            repo.branch().create("feature").run().await?;
            repo.checkout().branch("feature").run().await?;
            fs::write(repo.path().join("f.txt"), "feature").unwrap();
            repo.add().all().run().await?;
            let result = repo.commit().message("feature work").run().await?;
            Ok(result)
        }
    ...
    async fn async_full_workflow_init_to_merge() {
        ...
        fs::write(dir.join("README.md"), "# Project").unwrap();
        repo.add().all().run().await.unwrap();
        assert_eq!(repo.commit().message("initial").run().await.unwrap().branch, "main");
        repo.checkout().new_branch("feature").run().await.unwrap();
        fs::write(dir.join("feature.rs"), "fn f() {}").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("add feature").run().await.unwrap();
        repo.tag().create("v0.1.0-beta").message("beta").run().await.unwrap();
        repo.checkout().branch("main").run().await.unwrap();
        assert!(!repo.merge().branch("feature").no_ff().run().await.unwrap().fast_forward);
        repo.tag().create("v0.1.0").message("release").run().await.unwrap();
        assert!(dir.join("feature.rs").exists());
        assert_eq!(repo.branch().current().await.unwrap(), "main");
        assert!(repo.status().run().await.unwrap().is_clean());
    ...
    async fn async_full_workflow_clone_then_operate() {
        ...
        let origin_repo = fluent_git::git::init(&origin).initial_branch("main").run().await.unwrap().into_repo();
        origin_repo.config().set("user.name", "O").run().await.unwrap();
        origin_repo.config().set("user.email", "o@o").run().await.unwrap();
        fs::write(origin.join("README.md"), "# O").unwrap();
        origin_repo.add().all().run().await.unwrap();
        origin_repo.commit().message("init").run().await.unwrap();
        ...
        assert_eq!(clone_result.remote, "origin");
        let repo = clone_result.into_repo();
        repo.config().set("user.name", "C").run().await.unwrap();
        repo.config().set("user.email", "c@c").run().await.unwrap();
        fs::write(dest.join("f.rs"), "fn f() {}").unwrap();
        repo.add().all().run().await.unwrap();
        repo.commit().message("feature").run().await.unwrap();
        assert!(repo.remote().list().run().await.unwrap().iter().any(|r| r.name == "origin"));
